/* ============================================================
   ...
   Ported verbatim from the official Gut Check Brand Sheet v1 so the app
   matches the brand exactly. Pure string shapes (no <text>) so it rasterizes
   cleanly. See design/Gut Check Brand.html.
     gusSvg(expr)   expr: idle | happy | oops | sleepy
     gusMono(expr) · gusIcon() · gusBadge(id) · GUS_BADGES · GUS_COLORS
   ============================================================ */
const INK = '#1F2328',
  TEAL = '#00A79B',
  CREAM = '#FFF6EB',
...
function mouth(expr, ink) {
  ink = ink || INK
  if (expr === 'happy')
    return `<path d="M107 130 Q120 150 133 130 Z" fill="${ink}"/><path d="M114 142 Q120 150 126 142 Z" fill="${ink === INK ? ORANGE : ink}"/>`
  if (expr === 'oops') return `<ellipse cx="120" cy="134" rx="8" ry="9.5" fill="${ink}"/>`
  if (expr === 'sleepy')
    return `<path d="M114 132 q6 5 12 0" fill="none" stroke="${ink}" stroke-width="4.5" stroke-linecap="round"/>`
...
function cheeks(expr) {
  const y = 126,
    rx = expr === 'happy' ? 15 : 13.5,
    ry = expr === 'happy' ? 10 : 9
...
function extras(expr, mono) {
  if (expr === 'oops')
    return `<path d="M178 96 q7 11 0 18 a9 9 0 0 1 -11 -7 q4 -7 11 -11 z" fill="${mono ? CREAM : SKY}" stroke="${INK}" stroke-width="3"/>`
  if (expr === 'sleepy') return zee(168, 56, 1) + zee(184, 38, 1.4)
  if (expr === 'happy')
    return spark(176, 70, 1.05, mono ? CREAM : GOLD) + spark(58, 84, 0.85, mono ? CREAM : GOLD) + spark(120, 150, 0.5, mono ? CREAM : GOLD)
...
export function gusSvg(expr, opts) {
  expr = expr || 'idle'
  opts = opts || {}
  const w = opts.size || 240
  const uid = ++_uid
  const iris = 'iris' + uid
...
export function gusMono(expr, opts) {
  expr = expr || 'idle'
  opts = opts || {}
  const w = opts.size || 240
  const I = INK
...
export function gusIcon(opts) {
  ...
  for (let i = 0; i < 5; i++)
    for (let j = 0; j < 5; j++) {
      dots.push(`<circle cx="${30 + i * 46}" cy="${30 + j * 46}" r="3" fill="${CREAM}" opacity="0.12"/>`)
    }
...
export const GUS_BADGES = {
  food: { bg: GREEN, label: 'Fresh or Fake' },
  scam: { bg: TEAL, label: 'Scam Spotter' },
  math: { bg: ORANGE, label: 'Quick Math' },
  weird: { bg: GOLD, label: 'Weird Facts' },
  animal: { bg: CREAM, label: 'Animal Facts' },
  history: { bg: SAND, label: 'History Hits' },
...
function badgeGlyph(id) {
  switch (id) {
    case 'food':
      return `<ellipse cx="32" cy="34" rx="13" ry="17" fill="${CREAM}" stroke="${INK}" stroke-width="3.4"/>
    ...
    case 'scam':
      return `<circle cx="28" cy="28" r="11" fill="${CREAM}" stroke="${INK}" stroke-width="3.4"/>
    ...
    case 'math':
      return `<path d="M20 24 h10 M25 19 v10" stroke="${CREAM}" stroke-width="3.6" stroke-linecap="round"/>
    ...
    case 'weird':
      return `<path d="M32 14 L35 27 L48 24 L38 32 L48 40 L35 37 L32 50 L29 37 L16 40 L26 32 L16 24 L29 27 Z" fill="${CREAM}" stroke="${INK}" stroke-width="3" stroke-linejoin="round"/>
        <circle cx="32" cy="32" r="5" fill="${ORANGE}" stroke="${INK}" stroke-width="2.4"/>`
    case 'animal':
      return `<path d="M16 22 L24 30 L18 34 Z" fill="${ORANGE}" stroke="${INK}" stroke-width="2.8" stroke-linejoin="round"/>
    ...
    case 'history':
      return `<path d="M18 22 h28 l-3 5 h-22 z" fill="${TEAL}" stroke="${INK}" stroke-width="2.8" stroke-linejoin="round"/>
    ...
    case 'science':
      return `<path d="M27 18 v9 L18 44 a4 4 0 0 0 4 5 h20 a4 4 0 0 0 4 -5 L37 27 v-9 z" fill="${TEAL}" stroke="${CREAM}" stroke-width="3.2" stroke-linejoin="round"/>
...
export function gusBadge(id, opts) {
  opts = opts || {}
  const s = opts.size || 64,
    b = GUS_BADGES[id] || GUS_BADGES.food
...
export const GUS_COLORS = { INK, TEAL, CREAM, ORANGE, GREEN, GOLD, SAND }
